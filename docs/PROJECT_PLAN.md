...
### Chapter 5 (Informal Fallacies) — roster changes

- ~~**Add *appeal to ignorance* (*ad ignorantiam*) to Chapter 5.**~~ *Drafted 2026-08-17.* Added as the sixth and final fallacy of weak inductive support, framed as the limiting case of that group (reasoning from no evidence rather than from thin evidence). Carries the book's only treatment of *burden of proof*, with a Key Terms entry for both. Examples: McCarthy's Case 40 (flagged VERIFY — exact wording needs checking against the Tydings Committee record), 2020 election audits, and chemical-safety arguments running the same absence in both directions. Exercises added to Sets 2, 5, and 6, with odd-numbered solutions in `appendix-solutions.ptx`. Chapter count updated 16 → 17 in the source comment, §5.1 intro, and §5.5 recap. Per the lead author's note, the legitimate cousin is anchored on a large *undesigned* search (no dog has ever spoken English, despite millions of household-years of close attention), set explicitly against the empty loyalty file and the no-study-shows-harm arguments, which share its form and none of its substance. The Part IV pointer originally promised statistical power, which the book does not teach; it now points at test *sensitivity* in §15.5, which is the concept actually delivered. **Still open:** exam bank and Canvas question alignment — see §16.1 below.
- ~~**Cut the *fallacy of accent* forward reference in Chapter 4.**~~ *Done 2026-08-17.* The "What's coming" paragraph in `ch-language-and-meaning.ptx` now names equivocation as the one language-based fallacy and drops the accent promise. No further Ch. 4 work planned — nothing else in the book depends on it, per §3. Accent is out of scope: marginal in contemporary practice, and the ambiguity material carries its weight through equivocation.

### 16.1 Assessment materials trailing the Chapter 5 roster change

Adding appeal to ignorance put the book ahead of every assessment artifact that keys to the Chapter 5 roster. None of these are wrong as they stand — they simply cover 16 of 17 fallacies. Resolve before the roster is treated as stable:

- **Canvas item banks.** `canvas/build_quiz6.py` and `canvas/build_quiz7.py` between them cover all sixteen of the pre-existing fallacies (verified 2026-08-18) and none of appeal to ignorance. Both builders need items added and the `.imscc` packages rebuilt. The legitimate-cousin discrimination items matter more here than the identification items, since Canvas native question types handle multiple choice well and this fallacy's whole difficulty is discrimination.
- **Exam bank.** `Exam4_SpotTheFallacy_AdditionalProblems.md` and the Part I problem sets in the course folder predate the change and should get appeal-to-ignorance items on the same pass.
- **Ordering note.** Appeal to ignorance is now the *last* item in §5.3, after weak analogy. Any assessment material that walks the fallacies in book order needs its sequence checked, not just its coverage.
...
- **Chapter 5 §5.3** (appeal to ignorance, *legitimate cousin*) points forward to Part IV for the precise version of "would the evidence have shown up if it were there?" The pointer is an `<xref>` to `sec-ch15-sensitivity-specificity` and names test *sensitivity* as the formal counterpart. This is paid off: Chapter 15 defines sensitivity and specificity and works the cholesterol-threshold example. If that section is ever renamed or restructured, the xref and the sentence around it must move with it. Note that the book does **not** teach statistical power, so no forward reference should promise it.
...
1. *Accessibility.* The main text never relies on a diagram, so screen-reader users get a complete book without the appendix. The book's cross-cutting accessibility commitment from §7 holds cleanly.
