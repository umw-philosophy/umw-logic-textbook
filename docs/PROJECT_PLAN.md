...
- ~~**Add *appeal to ignorance* (*ad ignorantiam*) to Chapter 5.**~~ *Drafted 2026-08-17.* Added as the sixth and final fallacy of weak inductive support, framed as the limiting case of that group (reasoning from no evidence rather than from thin evidence). Carries the book's only treatment of *burden of proof*, with a Key Terms entry for both. Examples: McCarthy's Case 40 (flagged VERIFY — exact wording needs checking against the Tydings Committee record), 2020 election audits, and chemical-safety arguments running the same absence in both directions. Exercises added to Sets 2, 5, and 6, with odd-numbered solutions in `appendix-solutions.ptx`. Chapter count updated 16 → 17 in the source comment, §5.1 intro, and §5.5 recap. **Still open:** exam bank and Canvas question alignment; the forward reference to statistical power in Part IV should be checked once the statistical-reasoning chapter settles.
- ~~**Cut the *fallacy of accent* forward reference in Chapter 4.**~~ *Done 2026-08-17.* The "What's coming" paragraph in `ch-language-and-meaning.ptx` now names equivocation as the one language-based fallacy and drops the accent promise. No further Ch. 4 work planned — it remains non-load-bearing per §3. `book/source/chapters/ch-language-and-meaning.ptx` (~line 580) currently promises "plus relatives like the fallacy of accent and a few others" as language-based fallacies to come. Accent is not being taught, so the sentence should be rewritten to refer only to equivocation. Decision: accent is out of scope — marginal in contemporary practice, and the ambiguity material carries its weight through equivocation.
