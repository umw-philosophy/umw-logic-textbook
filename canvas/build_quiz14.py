#!/usr/bin/env python3
"""Build a Canvas Common Cartridge (.imscc) package for Quiz 14: Well-Formed Formulas and Main Operators."""
...
CANVAS = ROOT / "canvas"
BUILD = CANVAS / "build_quiz14_temp"
IMSCC_TARGET = CANVAS / "quiz14_wff_main_operator.imscc"
...
QUIZ = {
    "slug": "quiz14_wff_main_operator",
    "title": "Quiz 14: Well-Formed Formulas and Main Operators",
    "description": "Autograded multiple-choice quiz testing the identification of well-formed formulas (WFFs) and main operators (connectives) in sentential logic (Chapter 9).",
    "questions": [
        # Question 1 (WFF - Yes)
        (
            "Is the following symbolic statement a well-formed formula (WFF)?<br><br>"
            "<em>(P ⊃ Q) ∨ R</em>",
            [
                "Yes, it is a well-formed formula.",
                "No, it is not a well-formed formula.",
            ],
        ...
        # Question 4 (Main Operator)
        (
            "Identify the main operator (main connective) of the following well-formed formula:<br><br>"
            "<em>(A · B) ⊃ C</em>",
            [
                "The conditional (⊃)",
                "The conjunction (·)",
...
def qti_for_quiz(quiz: dict) -> str:
    ...
    for q_idx, (prompt, choices, correct) in enumerate(quiz["questions"], start=1):
        ...
        for c_idx, choice in enumerate(choices):
            ident = f"q14_{q_idx}_a{c_idx}"
            choice_xml.append(f"""
              <response_label ident="{ident}">
                <material><mattext texttype="text/html">{html.escape(choice)}</mattext></material>
              </response_label>""")
        correct_ident = f"q14_{q_idx}_a{correct}"
        items.append(f"""
      <item ident="q14_{q_idx}" title="Question {q_idx}">
        <itemmetadata>
          <qtimetadata>
