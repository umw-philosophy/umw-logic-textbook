#!/usr/bin/env python3
"""Build a Canvas Common Cartridge (.imscc) package for Quiz 17: Replacement Rules."""
...
CANVAS = ROOT / "canvas"
BUILD = CANVAS / "build_quiz17_temp"
IMSCC_TARGET = CANVAS / "quiz17_replacement_rules.imscc"
...
QUIZ = {
    "slug": "quiz17_replacement_rules",
    "title": "Quiz 17: Replacement Rules Next Line",
    "description": "Autograded multiple-choice quiz testing the replacement rules of natural deduction (Chapter 11).",
    "questions": [
        # Question 1 (DeM)
        (
            "<p>Given the following natural deduction proof, what is a valid next line using the replacement rules from Chapter 11?</p>"
        ...
        (
            "<p>Given the following natural deduction proof, what is a valid next line using the replacement rules from Chapter 11?</p>"
            '<table border="0" cellpadding="3" style="font-family: monospace; font-size: 1.1em;">'
            "<tr><td>1.</td><td>(A · B) ⊃ C</td><td></td></tr>"
            "</table>",
            [
                "2. A ⊃ (B ⊃ C) &nbsp;&nbsp;&nbsp;&nbsp; 1 Exp",
                "2. (A ⊃ B) ⊃ C &nbsp;&nbsp;&nbsp;&nbsp; 1 Exp",
...
def qti_for_quiz(quiz: dict) -> str:
    ...
    for q_idx, (prompt, choices, correct) in enumerate(quiz["questions"], start=1):
        ...
        for c_idx, choice in enumerate(choices):
            ident = f"q17_{q_idx}_a{c_idx}"
            choice_xml.append(f"""
              <response_label ident="{ident}">
                <material><mattext texttype="text/html">{html.escape(choice)}</mattext></material>
              </response_label>""")
        correct_ident = f"q17_{q_idx}_a{correct}"
        items.append(f"""
      <item ident="q17_{q_idx}" title="Question {q_idx}">
        <itemmetadata>
          <qtimetadata>
